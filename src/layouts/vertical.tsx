import {Layout,Sider,Header,Content,memo,Outlet,Head,boxShadow,Menu,useLayout} from "./rely";
import { RightOutlined, LeftOutlined } from "@ant-design/icons";
const Vertical = ({ items }) => {
  const { changeCollapsed, collapsed } = useLayout();
  return (
    <Layout className="h-m">
      <Header className="pl-0" style={{ boxShadow }}> <Head /></Header>
      <Layout>
        <Sider collapsed={collapsed} className=" relative">
          <Menu className=" overflow-y-auto h-full" defaultSelectedKeys={["1"]} mode="inline" items={items} />
